...
const THEMES = {
  dark: {
    bg1: "#0d0d0f", bg2: "#141418", grid: "#1e1e26",
    fill: "#c8b89a", text: "#e8e4dc", sub: "#6b6870", empty: "#1a1a20",
  },
  stone: {
    bg1: "#1c1810", bg2: "#231e12", grid: "#2e2416",
    fill: "#d4a96a", text: "#e8dcc8", sub: "#8a7a5a", empty: "#201c10",
  },
  fog: {
    bg1: "#e8e6e0", bg2: "#dddad3", grid: "#c8c5be",
    fill: "#3a3835", text: "#1a1816", sub: "#8a8780", empty: "#d0cdc6",
  },
  forest: {
    bg1: "#0e1a12", bg2: "#111e15", grid: "#1a2e1e",
    fill: "#6bcb77", text: "#c8e8cc", sub: "#4a7050", empty: "#141e16",
  },
  ocean: {
    bg1: "#0a1628", bg2: "#0d1e38", grid: "#142040",
    fill: "#5ab4e4", text: "#c8e4f8", sub: "#3a6880", empty: "#0e1c34",
  },
  rose: {
    bg1: "#1a0e0e", bg2: "#221212", grid: "#2a1818",
    fill: "#e4847a", text: "#f0d0cc", sub: "#7a4a48", empty: "#1e1010",
  },
  violet: {
    bg1: "#110820", bg2: "#1a1030", grid: "#2c1850",
    fill: "#c084fc", text: "#ede6ff", sub: "#7c5fa0", empty: "#1a1030",
  },
  gold: {
    bg1: "#160f00", bg2: "#221800", grid: "#382800",
    fill: "#f5c542", text: "#fff8e1", sub: "#a08030", empty: "#1e1500",
  },
  custom: {
    bg1: "#0b0b0d", bg2: "#141417", grid: "#232329",
    fill: "#d6c3a3", text: "#f2eee7", sub: "#7a7680", empty: "#1a1a1f",
  },
};
...
// These are the Y-fraction boundaries where the OS draws its own UI.
// Content must stay INSIDE safeTop..safeBot to avoid being covered.
//
//  Portrait (iOS / Android):
//    0.00 – 0.38  →  status bar + clock + date (iOS puts clock ~15–38%)
//    0.83 – 1.00  →  torch/camera buttons, home indicator, gesture bar
//
//  Landscape desktop — no lock-screen concern, full canvas used.
//
// An extra horizontal edge buffer handles iOS parallax zoom (~5% each side).

const SAFE = {
  portrait: {
    top:  0.40,   // first safe Y (below OS clock)
    bot:  0.83,   // last  safe Y (above home bar / torch buttons)
    edge: 0.06,   // extra left/right pad for parallax crop
  },
  landscape: {
    top:  0.10,
    bot:  0.92,
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (p.get("view") === "1") {
    ...
    let w, h;
    if (p.get("autosize") === "1") {
      // Use real physical pixels of whatever device opens the link
    ...
    const portrait    = h > w;
    const lockscreen  = p.get("autosize") === "1"; // lock screen mode = autosize

    const canvas = document.createElement("canvas");
    canvas.width  = w;
    canvas.height = h;
    drawWallpaper(canvas.getContext("2d"), w, h, true, portrait, lockscreen);
  ...
  if (p.get("download") === "1") {
    setTimeout(() => { render(); downloadWallpaper(); }, 300);
  }
});
...
function startCountdown() {
  const tick = () => {
    ...
    document.getElementById("countdown").textContent = `next update in ${h}:${m}:${s}`;
  };
  tick();
  setInterval(tick, 1000);
}
...
function setDevice(dev, el) {
  ...
  if (dev === "mac"     && !["2560x1600","1920x1080","3840x2160"].includes(sel.value)) sel.value = "2560x1600";
  if (dev === "windows" && !["2560x1600","1920x1080","3840x2160"].includes(sel.value)) sel.value = "1920x1080";
  render();
}
...
function setType(type, el) {
  ...
  el.classList.add("active");
  document.getElementById("lifeSection").style.display = type === "life" ? "" : "none";
  document.getElementById("yearSection").style.display = type === "year" ? "" : "none";
  document.getElementById("goalSection").style.display = type === "goal" ? "" : "none";
  render();
}
...
function onCustomAccent(val) {
  const theme = generateTheme(val);
  THEMES.custom = theme;
  // Sync <input type="color"> elements so buildWallpaperUrl() reads correct hex values
  const hexMap = {
    bg1: toHex(theme.bg1), bg2: toHex(theme.bg2), grid: toHex(theme.grid),
    fill: toHex(theme.fill), text: toHex(theme.text), sub: toHex(theme.sub),
    empty: toHex(theme.empty),
  };
  Object.entries(hexMap).forEach(([k, v]) => {
...
function render() {
  ...
  Object.entries(DEVICES).forEach(([dev, cfg]) => {
    ...
    c.width  = cfg.w;
    c.height = cfg.h;
    // Preview canvases: never lock-screen mode
    drawWallpaper(c.getContext("2d"), cfg.w, cfg.h, false, cfg.portrait, false);
  });
  const exp     = document.getElementById("exportCanvas");
  const portrait = !["mac", "windows"].includes(currentDevice);
  exp.width  = w;
  exp.height = h;
  // Export canvas: also not lock-screen (manual download uses full canvas)
  drawWallpaper(exp.getContext("2d"), w, h, true, portrait, false);
}

// ══ DRAW ══
// lockscreen = true  → restrict all content to OS-safe Y zone
// lockscreen = false → use full canvas (editor preview & manual download)
function drawWallpaper(ctx, W, H, hires, portrait, lockscreen = false) {
  ...
  if (currentType === "life") drawLifeCalendar(ctx, W, H, T, now, portrait, lockscreen);
  if (currentType === "year") drawYearCalendar(ctx, W, H, T, now, portrait, lockscreen);
  if (currentType === "goal") drawGoalCalendar(ctx, W, H, T, now, portrait, lockscreen);

  drawTimestamp(ctx, W, H, T, now, lockscreen, portrait);
}
...
// ── Resolve safe-zone layout values ──────────────────────────────────────────
// Returns { titleY, gridTop, gridBot, barY, barH, pad, safeH }
// All values are absolute pixels so each draw function stays clean.
function safeLayout(W, H, portrait, lockscreen) {
  const S = portrait ? SAFE.portrait : SAFE.landscape;

  // Extra horizontal padding when lock-screen (parallax buffer)
  const edgePad = lockscreen ? W * S.edge : 0;
  const pad     = W * 0.08 + edgePad;

  if (!lockscreen) {
    // Original layout — full canvas
    return {
      pad,
      titleY:  portrait ? H * 0.34 : H * 0.13,
      gridTop: portrait ? H * 0.38 : H * 0.30,
      gridBot: portrait ? H * 0.88 : H * 0.82,
      barY:    portrait ? H * 0.92 : H * 0.87,
      barH:    H * 0.008,
    };
  }

  // Lock-screen layout — content compressed into safe zone
  const sTop  = H * S.top;
  const sBot  = H * S.bot;
  const safeH = sBot - sTop;

  return {
    pad,
    titleY:  sTop + safeH * 0.06,          // title near top of safe zone
    gridTop: sTop + safeH * 0.16,          // grid starts after title + subtitle
    gridBot: sTop + safeH * 0.82,          // grid ends with room for bar
    barY:    sTop + safeH * 0.86,          // progress bar
    barH:    H * 0.006,
  };
}

// Same idea for goal calendar which has a more complex layout
function safeGoalLayout(W, H, portrait, lockscreen) {
  const S = portrait ? SAFE.portrait : SAFE.landscape;
  const edgePad = lockscreen ? W * S.edge : 0;
  const pad     = W * 0.10 + edgePad;

  if (!lockscreen) {
    return {
      pad,
      labelY:   portrait ? H * 0.34  : H * 0.10,
      numY:     portrait ? H * 0.45  : H * 0.42,
      dateRowY: portrait ? H * 0.52  : H * 0.56,
      gridBot:  portrait ? H * 0.91  : H * 0.88,
    };
  }
  ...
  return {
    pad,
    labelY:   sTop + safeH * 0.04,
    numY:     sTop + safeH * 0.28,
    dateRowY: sTop + safeH * 0.44,
    gridBot:  sTop + safeH * 0.95,
  };
}

// ── Life Calendar ─────────────────────────────────────────────────────────────
function drawLifeCalendar(ctx, W, H, T, now, portrait, lockscreen) {
  ...
  const { pad, titleY, gridTop, gridBot, barY, barH } =
    safeLayout(W, H, portrait, lockscreen);
  ...
  // Grid
  const gridH = gridBot - gridTop, gridW = W - pad * 2;
  const cols   = 52, rows = lifeYears;
  const cellW  = gridW / cols, cellH = gridH / rows;
  const gap    = Math.max(0.5, cellW * 0.12), r = Math.min(cellW, cellH) * 0.18;
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      let color;
      if (idx < weekLived) {
        color = lerpColor(T.fill, lighten(T.fill, 0.15), idx / Math.max(1, weekLived));
...
// ── Year Calendar ─────────────────────────────────────────────────────────────
function drawYearCalendar(ctx, W, H, T, now, portrait, lockscreen) {
  const mode       = document.getElementById("yearMode").value;
  const showMonths = document.getElementById("toggleMonths").classList.contains("on");
  const showToday  = document.getElementById("toggleToday").classList.contains("on");
  ...
  const { pad, titleY, gridTop, gridBot, barY, barH: barH2 } =
    safeLayout(W, H, portrait, lockscreen);
  ...
  const gridW = W - pad * 2, gridH = gridBot - gridTop;

  if (mode === "weeks") {
    const totalWeeks = 52;
    const weekOfYear = Math.floor((now - new Date(year, 0, 1)) / (7 * 24 * 3600 * 1000));
    const cols = portrait ? 13 : 26, rows = portrait ? 4 : 2;
    const cellW = gridW / cols, cellH = gridH / rows;
    const gap = cellW * 0.15, r = Math.min(cellW, cellH) * 0.25;
    ...
    const cols = portrait ? 25 : 50, rows = Math.ceil(totalDays / cols);
    const cellW = gridW / cols, cellH = gridH / rows;
    const gap = cellW * 0.12, r = Math.min(cellW, cellH) * 0.22;
    ...
    if (showMonths) {
      ...
      MONTHS.forEach((days, mi) => {
        ...
        const x = col * cellW, y = row * cellH - cellH * 0.1;
        if (y > 0) {
          ctx.fillStyle = T.sub;
...
// ── Goal Calendar ─────────────────────────────────────────────────────────────
function drawGoalCalendar(ctx, W, H, T, now, portrait, lockscreen) {
  ...
  const { pad, labelY, numY, dateRowY, gridBot } =
    safeGoalLayout(W, H, portrait, lockscreen);
  const cx = W / 2;
  ...
  const barH  = H * (portrait ? 0.008 : 0.010);
  const barW  = W - pad * 2;
  const barR  = barH / 2;
  ...
  for (let i = 0; i < totalDays; i++) {
    ...
    let color;
    if (i < daysDone) {
      color = lerpColor(T.fill, lighten(T.fill, 0.2), i / Math.max(1, daysDone));
    } else if (i === daysDone) {
      color = lighten(T.fill, 0.35);
      ctx.save();
      ctx.shadowColor = T.fill;
...
// ── Timestamp ─────────────────────────────────────────────────────────────────
function drawTimestamp(ctx, W, H, T, now, lockscreen, portrait) {
  const timeStr = now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
  const dateStr = now.toLocaleDateString("en", { weekday: "long", month: "long", day: "numeric" });

  let y;
  if (!lockscreen) {
    // Original: very bottom of canvas
    y = H * 0.975;
  } else {
    // Lock screen: just above the bottom safe boundary
    const S = portrait ? SAFE.portrait : SAFE.landscape;
    y = H * S.bot - H * 0.012;
  }
  ...
  ctx.fillText(`generated ${dateStr} at ${timeStr}`, W / 2, y);
}
...
function downloadWallpaper(mode = "download") {
  ...
  exp.width  = w;
  exp.height = h;
  // Manual download: never lock-screen mode (full canvas)
  drawWallpaper(exp.getContext("2d"), w, h, true, portrait, false);
  ...
  if (mode === "view") {
    ...
    const img = Object.assign(document.createElement("img"), {
      src: data,
      style: "max-width:100%;height:100vh;object-fit:contain;display:block;margin:auto;",
    });
...
function buildWallpaperUrl() {
  ...
  const params = new URLSearchParams({
    ...
    theme:    currentTheme,
    view:     1,
    autosize: 1,   // device measures itself at open time
  });
  ...
  if (currentType === "year") {
    params.set("yearMode",   document.getElementById("yearMode").value);
    params.set("showMonths", document.getElementById("toggleMonths").classList.contains("on") ? "1" : "0");
    params.set("showToday",  document.getElementById("toggleToday").classList.contains("on")  ? "1" : "0");
  }
  if (currentType === "goal") {
...
function copyUrl(spanId, btnId) {
  navigator.clipboard.writeText(document.getElementById(spanId).textContent).then(() => {
...
function applyUrlParams() {
  ...
  // Apply size FIRST so any render() triggered below uses correct dims
  if (p.get("w") && p.get("h")) {
  ...
  if (p.get("type")) {
    const c = document.querySelector(`.type-card[data-type="${p.get("type")}"]`);
  ...
  if (p.get("theme")) {
    if (p.get("theme") === "custom") {
      activateCustom();
      ["bg1","bg2","grid","fill","text","sub","empty"].forEach((k) => {
        if (p.get("c_" + k)) document.getElementById("cp_" + k).value = p.get("c_" + k);
      });
      applyCustomTheme();
    } else {
      const sw = document.querySelector(`.theme-swatch[data-theme="${p.get("theme")}"]`);
  ...
  if (p.get("goalDate"))  document.getElementById("goalDate").value  = p.get("goalDate");
  if (p.get("goalStart")) document.getElementById("goalStart").value = p.get("goalStart");
  if (p.get("goalTitle")) document.getElementById("goalTitle").value = p.get("goalTitle");

  if (p.get("yearMode")) document.getElementById("yearMode").value = p.get("yearMode");
  if (p.get("showMonths") !== null) {
    const btn = document.getElementById("toggleMonths");
    p.get("showMonths") === "1" ? btn.classList.add("on") : btn.classList.remove("on");
  }
  if (p.get("showToday") !== null) {
    const btn = document.getElementById("toggleToday");
    p.get("showToday") === "1" ? btn.classList.add("on") : btn.classList.remove("on");
  }
}
...
function getDayOfYear(date) {
  return Math.floor((date - new Date(date.getFullYear(), 0, 0)) / (24 * 3600 * 1000));
}
...
function lighten(hex, amount) {
  const c = parseHex(hex), cl = (v) => Math.min(255, Math.max(0, Math.round(v)));
...
function parseHex(hex) {
  // Handle rgb() strings returned by lighten()/darken()
